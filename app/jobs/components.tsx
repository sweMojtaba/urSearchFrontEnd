...
export function QueryJobs(query : {"query" : string}) {

    async function jobSearch(query: string) {
    ...
    jobSearch(query.query)
        ...
        .then((res) => localStorage.setItem("jobResultsList", JSON.stringify(res)))
    ...
    if (localStorage.getItem("jobResultsList") != null) {
        ...
        for (let idx = 0; idx < rawResults.length; idx++) {
            results.push({
                "company": rawResults[idx].labName,
                "location": "MADISON, WI", 
                "sigma": true, 
                "typeOfPosition": rawResults[idx].title
            })
        }
    }
