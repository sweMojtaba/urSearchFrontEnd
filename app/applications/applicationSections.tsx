...
import { JobCard } from "@/components/cards-and-items/cards"; // Importing custom JobCard and JobResultCard components from a specific directory
import { Col, Row } from "@/client-wrappers/bootstrap"; // Importing Bootstrap layout components (Container, Col, Row) from a custom wrapper
...
export function SearchBar() {
    return (
        <div>
            <input className="search-field" placeholder="&#x1F50E;&#xFE0E; SEARCH APPLICATIONS..." />
        ...
        <div className="more-opportunity-board">
            ...
            <Col>
                {moreOpportunitiesData.map((card, index) => (
                    <JobCard key={index} company={card.company} location={card.location} sigma={card.sigma} affiliation={card.affiliation} typeOfPosition={card.position} description={card.description} bookmarked={card.saved} /> // Rendering a JobCard component for each opportunity
                ))}
            </Col>
        </div>
    );
    ...
    const displayList = statusData.map((card, i) => (
        <Row key={i} className="job-results-card">
            <Col className="card-row">
                <Image src={"/uw-logo.png"} alt="UW Madison Logo" width={60} height={60} className="job-results-photo" /> {/* Rendering an image with the UW Madison logo */}
                <Col>
                    <h5>
                        <i>{card.company.toUpperCase()}</i>
                        {card.sigma && (
                            <Image src={"/sigma.png"} alt="Sigma" width={24} height={24} className="sigma-img" /> // Conditionally rendering a Sigma icon if the job is Sigma-affiliated
                        )}
                    </h5>
                    <h4>
                        <i>{card.position}</i>
                    </h4>{" "}
                    {/* Displaying the job position */}
                </Col>
            </Col>
            <Col lg={5} className="job-results-end">
                <Col>
                    ...
                    <h3>
                        <i>{card.status.toUpperCase()}</i>
                    </h3>{" "}
                    {/* Displaying the job status in uppercase */}
                </Col>
            </Col>
        </Row>
    ));
